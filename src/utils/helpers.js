...
export const handleLikeFunctionAsync = async (
  ...
  if (isTargetElementClicked(heartIcon, evt.target)) {
    ...
    callIfFunction(updateLikesFns);

    targetHeartIcon.setAttribute("data-liked", isLiked ? "false" : "true");

    setAttributes(targetHeartIcon, {
      src: isLiked ? inactiveIconSrc : activeIconSrc,
      alt: isLiked ? inactiveIconAlt : activeIconAlt,
    });
...
// helpers.js
const addFadeOutClass = (isMounted) => {
  return isMounted ? "" : "fade-out";
};

export { addFadeOutClass }; // Certifique-se de exportar a função corretamente
...
export function Component() {
  ...
  return (
    <div>
      {isMounted && (
        <div className={addFadeOutClass(isMounted)}>
          {/* Conteúdo do componente */}
        </div>
      )}

      <button onClick={handleUnmount}>Desmontar</button>
    </div>
  );
}
